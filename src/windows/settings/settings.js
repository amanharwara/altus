...
const { app, process, Menu, dialog } = require("electron").remote;
...
Array.from(settings.get("settings")).forEach((setting) => {
  ...
  let settingElement = document.createRange()
    .createContextualFragment(`<div class="setting" data-setting="${
    setting.id
  }" data-value="${setting.value ? "enabled" : "disabled"}">
...
document.querySelector(".save.button").addEventListener("click", (e) => {
  ...
  document.querySelector(".save.button").innerHTML =
    '<span class="lni lni-spinner-solid"></span>';
    ...
    .then((result) => {
      ...
      if (!canceled) {
        const data = new Uint8Array(
          Buffer.from(JSON.stringify(settings.store, null, "\t"))
        );
