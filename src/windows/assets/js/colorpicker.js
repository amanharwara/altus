...
function KellyColorPicker(ca) {
    function S() {
        ...
        w.draw = function() {
            var a, b;
            if (this.imageData[B]) e.putImageData(this.imageData[B], this.position.x, this.position.y);
            else {
                var c = Y(this.color);
                v.width = this.size;
                v.height = this.size;
                ...
                "triangle" == B && (d = "quad");
                f.beginPath();
                35 > this.size ? b = a = v.width / 2 : (a = v.width / 2 - this.lineWidth, f.arc(this.size / 2, this.size / 2, a, 0, 2 * H), f.strokeStyle = "rgba(0, 0, 0, 0.4)", f.lineWidth = this.lineWidth, f.stroke(), b = a - 6, f.closePath(), f.beginPath(), f.arc(this.size / 2, this.size / 2, b, 0, 2 * H), f.strokeStyle = "rgba(0, 0, 0, 0.4)", f.lineWidth = this.lineWidth, f.stroke(), f.closePath());
                f.beginPath();
                if ("quad" == d) d = Math.floor((2 * b - 4) / Math.sqrt(2)), b = (this.size - d) / 2,
                    a = b + d, b = b + d / 2 - d / 2, f.moveTo(a, b), f.lineTo(a - d, b), f.lineTo(a - d, b + d), f.lineTo(a, b + d);
                else {
                    d = Math.floor((2 * b - 4) * Math.sin(H / 180 * 60));
                    a = 2 * b + (a - b);
                    b = this.size / 2;
    ...
    function V(a, b, c) {
        ...
        d.isDotIn = function(a) {
            ...
            for (var c = 0; c <= b.length - 1; ++c) b[c].x += this.paddingX, b[c].y += this.paddingY;
            c = this.calcS(b);
            a = [{
                x: b[0].x,
                y: b[0].y
        ...
        d.draw = function() {
            ...
            f.beginPath();
            "left" == this.align && (f.moveTo(this.lineWidth /
                2, this.width - this.lineWidth), f.lineTo(this.width, this.width - this.lineWidth), f.lineTo(this.lineWidth, this.lineWidth), f.lineTo(this.lineWidth, this.width - this.lineWidth));
            ...
            this.selected && (f.fillStyle = "rgba(255,255,255, 1)", f.fill(), f.strokeStyle = "rgba(0, 0, 0, 1)",
                f.stroke(), f.closePath(), f.beginPath(), f.lineWidth = this.lineWidth, "left" == this.align && (f.moveTo(this.selectSize, this.width - this.selectSize), f.lineTo(this.width - 2 * this.selectSize, this.width - this.selectSize), f.lineTo(this.selectSize, 2 * this.selectSize), f.lineTo(this.selectSize, this.width - this.selectSize)), "right" == this.align && (f.moveTo(2 * this.selectSize, this.width - this.selectSize), f.lineTo(this.width - this.selectSize, this.width - this.selectSize), f.lineTo(this.width - this.selectSize, 2 * this.selectSize), f.lineTo(2 *
                    this.selectSize, this.width - this.selectSize)));
            var a = Y(this.color);
    ...
    function R(a) {
        var b = a.getBoundingClientRect(),
            c = 0,
            d = 0;
        D && (d = r.width + 2 * r.padding);
        a === n ? b.width <= b.height ? c = b.height : b.height < b.width && (c = b.width) : Z ? "height" == Z ? c = b.height : "width" == Z && (c = b.width) :
            b.width > b.height ? c = b.height : b.height >= b.width && (c = b.width);
        c = parseInt(c);
        D && (c -= d);
        return 0 >= c ? !1 : c
    }
    ...
    function N(a, b) {
        var c = 1,
            d = !1;
        a = a.trim(a);
        if (7 >= a.length) "#" == a.charAt(0) && (a = a.slice(1)), 3 == a.length ? d = a + a : 6 == a.length && (d = a);
            ...
            if (3 <= e.length && 4 >= e.length) {
                ...
                if (255 >= f.r && 255 >= f.g && 255 >= f.b && (d = da(f), 4 == e.length)) {
                    c = parseFloat(e[3]);
                    if (!c ||
                        0 > c) c = 0;
                    1 < c && (c = 1)
    ...
    function ha() {
        ...
        a.draw = function() {
            ...
            e.lineWidth = 2;
            for (a = 0; a <= this.path.length - 1; ++a) 0 == a ? e.moveTo(this.path[a].x, this.path[a].y) : e.lineTo(this.path[a].x, this.path[a].y);
            e.stroke();
            e.closePath()
        };
        a.updateSize = function() {
            ...
            for (var a = 0; a <= this.path.length - 1; ++a) this.path[a].x += h.pos.x, this.path[a].y += h.pos.y
        };
        a.isDotIn = function(a) {
            return a.x < this.path[0].x || a.x > this.path[0].x + this.size || a.y < this.path[0].y || a.y > this.path[0].y + this.size ?
                !1 : !0
        };
        return T.quad = a
    }
    ...
    function ia() {
        ...
        a.limitDotPosition = function(a) {
            ...
            var g = (this.path[0].y - this.path[1].y) / (this.path[0].x - this.path[1].x),
                ...
                g = Math.floor(this.path[2].y + g * (e -
                    this.path[2].x));
            c < d && (f = a);
        ...
        a.draw = function() {
            ...
            v.width = this.size;
            v.height = this.size;
            for (var a = this.path[1].x,
                    c = this.path[1].y, d = 0, g = 0; g < this.size; g++)
                for (var h = 0; h < this.size; h++) {
                    var k = {
                        x: this.path[1].x + h,
                        y: this.path[1].y + g
                    };
                    y.isDotIn(k) ? (k = this.dotToSv(k), k = U(m.h, k.s, k.v), this.imageData.data[d + 0] = k.r, this.imageData.data[d + 1] = k.g, this.imageData.data[d + 2] = k.b, this.imageData.data[d + 3] = 255) : (this.imageData.data[d + 0] = 0, this.imageData.data[d + 1] = 0, this.imageData.data[d + 2] = 0, this.imageData.data[d + 3] = 0);
                    d += 4
                }
            ...
            e.lineWidth =
                2;
            a = this.path;
            for (d = 0; d <= a.length - 1; ++d) 0 == d ? e.moveTo(a[d].x, a[d].y) : e.lineTo(a[d].x, a[d].y);
            e.stroke();
            e.closePath()
        };
        a.calcS = function(a) {
        ...
        a.dotToSv = function(a) {
            ...
            1 < d && (d = 1);
            var e = this.vol[0],
                c = Math.sqrt(Math.pow(c[0].x + d * (c[1].x - c[0].x) - e.x, 2) + Math.pow(c[0].y + d * (c[1].y -
                    c[0].y) - e.y, 2));
            1 > c && (c = Math.floor(c));
            c > this.h - 1 && (c = this.h);
            c /= this.h;
        ...
        a.updateSize = function() {
            this.outerRadius = h.innerRadius - x.paddingX -
                this.padding;
            this.size = Math.floor(2 * this.outerRadius * Math.sin(H / 180 * 60));
            ...
            for (a = 0; a <= this.path.length - 1; ++a) this.path[a].x += h.pos.x, this.path[a].y += h.pos.y;
            this.vol = [];
            ...
            this.sOnTop ? (a = ka(this.path[0], this.path[2]),
                this.vol[0] = {
                    ...
                    x: a.x,
                    y: a.y
                }, this.sSide = this.path[1]) : (a = ka(this.path[0], this.path[1]), this.vol[0] = {
                x: this.path[2].x,
                y: this.path[2].y
            }, this.vol[1] = {
                x: a.x,
    ...
    function z(a, b, c, d) {
        ...
        d || (d = "");
        O[d + b] = c;
        a.addEventListener ? a.addEventListener(b, O[d + b]) : a.attachEvent("on" + b, O[d + b]);
        return !0
    }
    ...
    function q(a, b, c) {
        ...
        c || (c = "");
        if (!O[c + b]) return !1;
        a.removeEventListener ? a.removeEventListener(b, O[c + b]) : a.detachEvent("on" + b, O[c + b]);
        O[c + b] = null;
        return !0
    }
    ...
    function ja(a, b, c) {
        ...
        a = 180 * Math.atan2(a.y - b.y, a.x - b.x) / H;
        c && 0 > a && (a = 360 + a);
        return a
    }
    ...
    function ea() {
        ...
        var a = u;
        D && (a += r.width + 2 * r.padding);
        "CANVAS" != E.tagName && (E.style.width = a + "px", E.style.height = u + "px");
        n.width = a;
        n.height = u;
        P != n && (n.style.width = a + "px", n.style.height = u + "px");
    ...
    function W(a) {
        if (t && (!l.updateinput || (0, l.updateinput)(g, t, a))) {
            var b = "rgba(" + F.r + ", " + F.g + ", " + F.b + ", " + G.toFixed(2) + ")";
            a || (t.value = 1 > G && "mixed" === aa ? b : "hex" === aa || "mixed" === aa ? I : b);
    ...
    function ta() {
        if (!E) return !1;
        "CANVAS" != E.tagName ? (n = document.createElement("CANVAS"), E.appendChild(n)) : n = E;
    ...
    function Q(a) {
        a = a || window.event;
        var b, c = document.body.scrollLeft + document.documentElement.scrollLeft,
    ...
    function pa(a) {
        for (var b = !1, c = 0; c <= k.length - 1; ++c) k[c].selected && (b = c), k[c].selected = !1;
        ...
        d && l.selectcolorsaver && (0, l.selectcolorsaver)(g,
            k[a]);
    ...
    function fa() {
        for (var a = 0; a <= k.length - 1; ++a) k[a].selected && (k[a].color = I)
    }

    function qa() {
        if (k.length)
            for (var a = 0; a <= k.length - 1; ++a) k[a].draw()
    }

    function ua() {
        if (!e) return !1;
        e.clearRect(0, 0, n.width, n.height);
        if (L) return e.putImageData(X, 0, 0), qa(), !0;
        h.draw();
        y.draw();
        D && r.draw();
        qa();
        w && w.draw();
        A || (X = e.getImageData(0, 0, n.width, n.height), L = !0);
        return !0
    }
    ...
    function M() {
        ...
        D && (e.beginPath(), e.rect(r.pos.x - 2, r.padding + r.height *
            (1 - G) - 1, r.width + 4, 2), e.strokeStyle = "rgba(0,0,0, 0.8)", e.lineWidth = 2, e.stroke(), e.closePath());
        e.beginPath();
        var b = x.path,
            c;
        c = H / 180 * a;
        for (var a = [], d = 0; d <= b.length - 1; ++d) a[d] = {
            x: b[d].x * Math.cos(c) - b[d].y * Math.sin(c),
            y: b[d].x * Math.sin(c) + b[d].y * Math.cos(c)
        };
        for (b = 0; b <= a.length - 1; ++b) a[b].x += h.pos.x, a[b].y += h.pos.y, 0 == b ? e.moveTo(a[b].x, a[b].y) : e.lineTo(a[b].x, a[b].y);
    ...
    h.draw = function() {
        if (this.imageData) e.putImageData(this.imageData,
            ...
            e.beginPath();
            e.arc(J, J, this.innerRadius, 0, 2 * H);
            e.fill();
    ...
    C.initSvCursor = function() {
        if (!n) return !1;
        this.curType = 1;
        this.stCursor || (this.stCursor = window.getComputedStyle(this.cEl).cursor,
        ...
        f.beginPath();
        f.lineWidth = 2;
        f.arc(a / 2, a / 2, this.size / 2, 0, 2 * H);
        f.stroke();
        f.closePath();
        var b = v.toDataURL();
        this.svCursorData = "url(" + b + ") " + a / 2 + " " + a / 2 + ", auto";
        if (!this.svCursorData) return !1;
        this.cEl.style.cursor = this.svCursorData; - 1 === this.cEl.style.cursor.indexOf(b) &&
            (this.svCursorData = "crosshair", this.cEl.style.cursor = "crosshair");
        return !0
    };
    ...
    this.setColorByHex = function(a, b) {
        ...
        D && a == I && L && c != G ? (G = c, M()) : I && a == I && L || (G = c, F = Y(a), I = a, m = la(F), c = y.svToDot(m),
            m.x = c.x, m.y = c.y, L = !1, fa(), M(), l.change && (0, l.change)(g), W(b))
    };
    ...
    this.mouseDownEvent = function(a) {
        ...
        var b, c = !1,
            d = Q(a);
        if (h.isDotIn(d)) A = "wheel", g.setHueByDot(d), b = function(a) {
            g.wheelMouseMove(a, d)
        }, c = function(a) {
            KellyColorPicker.cursorLock = !1;
            g.wheelMouseUp(a, d)
        };
        else if (y.isDotIn(d)) A = "sv", g.setColorByDot(d), b = function(a) {
            g.svMouseMove(a, d)
        }, c = function(a) {
            KellyColorPicker.cursorLock = !1;
            g.svMouseUp(a, d)
        };
        else if (D && r.isDotIn(d)) A = "alpha", g.setAlphaByDot(d), b = function(a) {
    ...
    this.alphaMouseMove = function(a,
        b) {
        a.preventDefault();
        if (A && K) {
            K = !1;
    ...
    this.getColorSaver = function(a) {
        for (var b = 0; b <= k.length - 1; ++b)
            if (!a && k[b].selected || k[b].align == a) return k[b].rgb = Y(k[b].color), k[b].hsv = la(k[b].rgb.r, k[b].rgb.g, k[b].rgb.b), k[b]
    };
    ...
    this.destroy = function() {
        ...
        t && (q(t, "click", "input_edit_"), q(t, "change", "input_edit_"), q(t, "keyup", "input_edit_"), q(t, "keypress", "input_edit_"));
        KellyColorPicker.popupEventsInclude && O.popup_close_click && (KellyColorPicker.activePopUp &&
            KellyColorPicker.activePopUp.popUpClose(!1), q(document, "click", "popup_close_"), q(window, "resize", "popup_resize_"), KellyColorPicker.popupEventsInclude = !1);
        h.imageData = null;
        v = X = y.imageData = null;
        E && E.parentNode && E.parentNode.removeChild(E);
        P && q(window, "resize", "canvas_");
        oa();
        g = null
    };
    (function(a) {
        ...
        a.input && "object" !== typeof a.input ?
            (a.input = document.getElementById(a.input), t = a.input) : "object" === typeof a.input && (t = a.input);
        void 0 !== a.changeCursor && (sa = a.changeCursor);
        void 0 !== a.alpha && (G = a.alpha);
        void 0 !== a.alphaSlider && (D = a.alphaSlider);
        void 0 !== a.inputColor && (na = a.inputColor);
        void 0 !== a.inputFormat && (aa = a.inputFormat);
        a.userEvents && (l = a.userEvents);
        a.place && "object" !== typeof a.place && (c = a.place, a.place = document.getElementById(a.place));
        a.place ? E = a.place : t ? (p.tag = document.createElement("div"), p.tag.className = "popup-kelly-color",
            a.popupClass ? p.tag.className = a.popupClass : (p.tag.className = "popup-kelly-color", p.tag.style.position = "absolute", p.tag.style.bottom = "0px", p.tag.style.left = "0px", p.tag.style.display = "none", p.tag.style.backgroundColor = "#e1e1e1", p.tag.style.border = "1px solid #bfbfbf", p.tag.style.boxShadow = "7px 7px 14px -3px rgba(0,0,0,0.24)", p.tag.style.borderTopLeftRadius = "4px", p.tag.style.borderTopRightRadius = "4px", p.tag.style.borderBottomLeftRadius = "4px", p.tag.style.borderBottomRightRadius = "4px", p.tag.style.padding =
                "12px", p.tag.style.boxSizing = "content-box"), E = p.tag, document.getElementsByTagName("body")[0].appendChild(p.tag), z(t, "click", function(a) {
        ...
        a.resizeWith && ("object" !== typeof a.resizeWith && "boolean" !== typeof a.resizeWith && (a.resizeWith = document.getElementById(a.resizeWith)),
            P = !0 === a.resizeWith ? n : a.resizeWith, a.resizeSide && (Z = a.resizeSide), P && (R(P) && (a.size = R(P)), z(window, "resize", function(a) {
                return g.syncSize(a)
            }, "canvas_")));
        a.size && 0 < a.size && (u = a.size);
        b ? "undefined" !== typeof console && console.log("KellyColorPicker : " + b) : ("quad" == B && (y = ha()), "triangle" == B && (y = ia()), t && (b = function(a) {
            a = a || window.event;
            a.target || (a.target = a.srcElement);
            g.setColorByHex(a.target.value, !0)
        }, z(t, "click", b, "input_edit_"), z(t, "change", b, "input_edit_"), z(t, "keyup", b, "input_edit_"), z(t, "keypress",
            b, "input_edit_")), a.colorSaver && (V("left", !0), V("right")), a.methodSwitch && S(), ba(), ea(), g.setColorByHex(!1))
    })(ca)
}
KellyColorPicker.cursorLock = !1;
...
KellyColorPicker.attachToInputByClass = function(ca, S) {
    for (var V = [], R = document.getElementsByClassName(ca), N = 0; N < R.length; N++) S ? S.input = R[N] : S = {
        input: R[N],
        size: 150
