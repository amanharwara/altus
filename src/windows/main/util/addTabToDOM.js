...
function addTabToDOM(tab_id, tabName) {
  ...
  let tabContentElement = document
    ...
    .createContextualFragment(
      `<div id="tab-content-${tab_id}"><webview id="whatsapp-${tab_id}" preload="./whatsapp.js" src="https://web.whatsapp.com/" useragent="${window.navigator.userAgent.replace(
        ...
        ""
      )}" partition="persist:${tab_id}" webpreferences="spellcheck=${
        tabSettings.spellcheck ? 1 : 0
      }"></webview></div>`
    );
  ...
  document
    .querySelector(`#whatsapp-${tab_id}`)
    .addEventListener("dom-ready", () => {
      setTabTheme(document.querySelector(`#whatsapp-${tab_id}`), themeName);
      ...
      Swal.fire({
        ...
        html: `<div class="inputs">
                    ...
                    <div class="toggle-field">
                        ...
                        <div class="input-checkbox">
                            <input type="checkbox" id="${tab_id}-spellcheck" class="checkbox" />
                            <div class="toggle-bg"></div>
                        </div>
                    </div>
                </div>`,
        ...
        onRender: () => {
          ...
          document.getElementById(`${tab_id}-utility-bar-toggle`).checked =
            tabSettings.utility_bar;
          document.getElementById(`${tab_id}-spellcheck`).checked =
            tabSettings.spellcheck;
        ...
        if (result.value) {
          ...
          let utility_bar = document.getElementById(
            `${tab_id}-utility-bar-toggle`
          ).checked;
          let tab_color = document.getElementById(`${tab_id}-tab-color`).value;
          let theme = document
            .getElementById(`${tab_id}-theme-value`)
            .getAttribute("data-selection-value");
          let spellcheck = document.getElementById(`${tab_id}-spellcheck`)
            .checked;
          ...
          if (theme !== tabInList.theme) {
            setTabTheme(document.querySelector(`#whatsapp-${tab_id}`), theme);
          }
          ...
          if (spellcheck !== tabInList.spellcheck) {
            location.reload();
          }
        }
      });
